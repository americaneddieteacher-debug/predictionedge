...
> ✅ **2026-07-14 Claude 검증 완료:** 백엔드는 100% 정상. anon 키(아래)는 Supabase에서
> 현재 활성(disabled=false)으로 확인, 마이그레이션 054~067 전부 프로덕션 적용 확인,
> 보안 진단 ERROR 0건. **따라서 "Invalid API key"의 원인은 오직 Railway 빌드에 이
> 공개키가 안 구워진 것 하나뿐.** 남은 조치는 아래 값 확인 + Redeploy 한 번.

**할 일 (브라우저 어시스턴트에게 시켜도 됨):**
1. Railway → BIG → **Variables**에서 아래 두 개가 정확히 이 값인지 대조
   - `NEXT_PUBLIC_SUPABASE_URL` = `https://dikdzguigukiyepunruu.supabase.co`
   - `NEXT_PUBLIC_SUPABASE_ANON_KEY` = 아래 값 그대로 (공개키라 노출돼도 안전, 브라우저 번들에 실림)
     ```
     eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRpa2R6Z3VpZ3VraXllcHVucnV1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMzc4NDUsImV4cCI6MjA5MTgxMzg0NX0.Ga_Yd7cKnb5qxzmnVdyjgzkdrp4kN51wwWO52IrEkw4
     ```
     (끝 6자 `IrEkw4`, `role:anon`. 혹시 `service_role` 키가 잘못 들어가 있으면 그게 바로 원인 → 위 값으로 교체)
2. 값이 맞든 틀리든 → **Deployments → 최신 배포 ⋯ → Redeploy** (필수 — `NEXT_PUBLIC_*`는 빌드타임에 구워지므로 재배포 없으면 절대 안 고쳐짐)
